...
const setupRequestModule = async (options: SetupOptions = {}) => {
  ...
  vi.doMock('../../src/mobile-core/auth', () => ({
    getToken: getTokenMock,
    removeToken: removeTokenMock
  }));

  vi.doMock('../../src/mobile-core/env', () => ({
    mobileEnv
  }));

  vi.doMock('../../src/mobile-core/crypto', () => ({
  ...
  vi.doMock('../../src/mobile-core/rsa', () => ({
    decrypt: decryptMock,
    encrypt: encryptMock
  }));

  const requestModule = await import('../../src/mobile-core/request');
  const errorsModule = await import('../../src/mobile-core/errors');
