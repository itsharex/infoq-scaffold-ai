...
export async function runRunner({ suite = 'smoke', report = false } = {}) {
  ...
  try {
    reactMounted = await miniProgram.evaluate(() => Boolean(getApp() && getApp().$app));
  } catch (error) {
    const detail = formatError(error);
    exceptionLogs.push({
      message: 'React runtime probe failed.',
      stack: detail,
      timestamp: new Date().toISOString()
    });
    log(`[APP_EXCEPTION] React runtime probe failed: ${detail}`);
  }
  if (!reactMounted) {
    log('React runtime probe is not mounted in current automator session; route assertions continue with selector checks best-effort.');
  }
