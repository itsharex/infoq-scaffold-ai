...
FRONTEND_DIR="${REPO_ROOT}/infoq-scaffold-frontend-react"
BACKEND_JAR="${BACKEND_DIR}/infoq-admin/target/infoq-admin.jar"

BACKEND_PORT="${BACKEND_PORT:-8080}"
FRONTEND_HOST="${FRONTEND_HOST:-127.0.0.1}"
FRONTEND_PORT="${REACT_PORT:-5174}"
PROFILE="${PROFILE:-dev}"
...
LOG_DIR="/tmp/infoq-react-runtime-verification"
mkdir -p "${LOG_DIR}"
BACKEND_LOG="${LOG_DIR}/backend-${BACKEND_PORT}.log"
FRONTEND_LOG="${LOG_DIR}/frontend-react-${FRONTEND_PORT}.log"
...
if [[ "${FRONTEND_ONLY}" -ne 1 ]]; then
  ...
  if [[ -n "${BACKEND_PID}" && "${FORCE_RESTART}" -eq 1 ]]; then
    echo "[react-runtime] restarting backend on port ${BACKEND_PORT} (pid=${BACKEND_PID})"
    kill_pid_if_running "${BACKEND_PID}"
    BACKEND_PID=""
  fi

  if [[ "${BUILD_BACKEND}" -eq 1 || ! -f "${BACKEND_JAR}" ]]; then
    echo "[react-runtime] building backend jar..."
    (
      cd "${BACKEND_DIR}"
  ...
  if [[ ! -f "${BACKEND_JAR}" ]]; then
    echo "[react-runtime] backend jar not found: ${BACKEND_JAR}" >&2
    exit 1
  fi

  if [[ -z "${BACKEND_PID}" ]]; then
    echo "[react-runtime] starting backend on :${BACKEND_PORT}"
    ...
    if ! wait_http "http://127.0.0.1:${BACKEND_PORT}/auth/code" "${WAIT_SECONDS}"; then
      echo "[react-runtime] backend failed to become ready. log=${BACKEND_LOG}" >&2
      tail -n 120 "${BACKEND_LOG}" >&2 || true
      exit 1
    fi
    echo "[react-runtime] backend ready: http://127.0.0.1:${BACKEND_PORT}/auth/code (pid=${STARTED_BACKEND_PID})"
  else
    echo "[react-runtime] backend already running on :${BACKEND_PORT} (pid=${BACKEND_PID})"
  fi
fi
...
if [[ "${BACKEND_ONLY}" -ne 1 ]]; then
  ...
  if [[ -n "${FRONTEND_PID}" && "${FORCE_RESTART}" -eq 1 ]]; then
    echo "[react-runtime] restarting React admin on port ${FRONTEND_PORT} (pid=${FRONTEND_PID})"
    kill_pid_if_running "${FRONTEND_PID}"
    FRONTEND_PID=""
  fi

  if [[ -n "${FRONTEND_PID}" ]]; then
    echo "[react-runtime] React admin already running on :${FRONTEND_PORT} (pid=${FRONTEND_PID})"
  else
    echo "[react-runtime] starting React admin on ${FRONTEND_HOST}:${FRONTEND_PORT}"
    ...
    if ! wait_http "http://${FRONTEND_HOST}:${FRONTEND_PORT}/" "${WAIT_SECONDS}"; then
      echo "[react-runtime] React admin failed to become ready. log=${FRONTEND_LOG}" >&2
      tail -n 120 "${FRONTEND_LOG}" >&2 || true
      exit 1
    fi

    echo "[react-runtime] React admin ready: http://${FRONTEND_HOST}:${FRONTEND_PORT}/ (pid=${STARTED_FRONTEND_PID})"
  fi
fi
...
echo "[react-runtime] state file: ${STATE_FILE}"
echo "[react-runtime] backend log: ${BACKEND_LOG}"
if [[ "${BACKEND_ONLY}" -ne 1 ]]; then
  echo "[react-runtime] frontend log: ${FRONTEND_LOG}"
fi
